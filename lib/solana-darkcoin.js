...
//
// The mint comes ONLY from lib/token-config.js (env TOKEN_MINT_ADDR). Until
// darkcoin is minted the chain layer soft-disables: init() warns and returns
// false, and every on-chain call throws a clear error instead of touching a
// token that isn't ours.
// ============================================================================
...
const { TOKEN_MINT_ADDR, TOKEN_PUMP_URL, TOKEN_DECIMALS, TOKEN_LIVE, TOKEN_PROGRAM } = require('./token-config');
// The old styxx-era mint was Token-2022; pump.fun's standard mints are classic
// SPL. Selected via env TOKEN_PROGRAM ('spl' default | 'token2022').
const CITY_TOKEN_PROGRAM_ID = TOKEN_PROGRAM === 'token2022' ? TOKEN_2022_PROGRAM_ID : TOKEN_PROGRAM_ID;
...
function _requireChain() {
  if (!chainReady) {
    throw new Error('[darkcoin] on-chain layer disabled — TOKEN_MINT_ADDR is not set (mint pending launch)');
  }
}

function init() {
  if (!TOKEN_LIVE) {
    console.warn('[darkcoin] TOKEN_MINT_ADDR not set — mint pending; on-chain layer disabled, DB economy only');
    return false;
  }
  const rpcUrl = process.env.SOLANA_RPC_URL || 'https://api.mainnet-beta.solana.com';
  connection = new Connection(rpcUrl, 'confirmed');
  tokenMint = new PublicKey(TOKEN_MINT_ADDR);
...
async function _withRpcRetry(label, fn, { max = 6, baseMs = 600 } = {}) {
  ...
  for (let i = 0; i < max; i++) {
    try { return await fn(); }
      ...
      console.warn(`[darkcoin:${label}] 429 retry ${i+1}/${max} in ${wait}ms`);
...
// ─── On-chain burn — destroys $DARKCOIN from treasury's ATA permanently ────
...
async function getTreasuryBalances() {
  ...
  const tokenBal = await getDarkcoinBalance(treasury.publicKey.toBase58());
  return {
    pubkey: treasury.publicKey.toBase58(),
    sol: sol / LAMPORTS_PER_SOL,
    // Legacy field name: consumed as `.styxx` by server.js, arena-ui,
    // darkcoin-live and darkcoin-flow. Renaming is an API change owed to the
    // schema-rename migration, not this pass.
    styxx: tokenBal,
  };
}
